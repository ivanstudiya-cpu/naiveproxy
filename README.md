...
<details>
<summary><b>v4.2.2</b> — Security Audit ← ТЕКУЩАЯ</summary>

**🔴 Критичные фиксы:**
- 🔒 SSH порт теперь меняется правильно на Ubuntu 22.04+ (поддержка `sshd_config.d/`)
- 🔒 Авто-отключение `ssh.socket` (Ubuntu 22.04+ override)
- 🛡️ Защита от удаления последнего домена (раньше ломало сервер)
- 📦 `apt update` перед установкой Fail2Ban (фикс установки на чистом сервере)
- 🔧 `PasswordAuthentication` и `PermitRootLogin` — добавление если строки нет в конфиге

**🟡 Улучшения безопасности:**
- ⚡ Fail2Ban: `banaction = iptables-multiport` (быстрее чем UFW при тысячах банов)
- 🌐 UFW: `allow 80/tcp` вместо `limit` (фикс ACME сертификатов Let's Encrypt)
- 🔑 Генерация паролей: `[a-zA-Z0-9_-]`, 20 символов (раньше 16)
- ♻️ Caddy `Restart=on-failure` + `RestartSec=5s` (авто-восстановление при сбое)

**🐛 Исправлено в боте:**
- ✅ `/qr` теперь работает — фикс curl `-F` + `--data-urlencode` конфликта
- ✅ `/adduser` теперь корректно валидирует логин и пароль
- ✅ Команды чистятся от `\r\n` (Telegram добавлял невидимые символы)
- ✅ `set +e` внутри обработчика — ошибка одной команды не ломает бот
- ✅ Авто-установка `qrencode` если не установлен
- ✅ Авто-установка `binutils` для проверки naive padding

**🔍 Диагностика:**
- 🐛 Фикс счётчиков (`((pass++))` → `pass=$((pass+1))` — `set -e` ломал диагностику)
- 🐛 Фикс ALPN проверки (добавлен `-servername` для SNI)
- 🐛 Фикс подсчёта ошибок в журнале (`grep -c | echo "0"` возвращал две строки)
- 🐛 Улучшена проверка naive padding (несколько критериев, не только `^Padding$`)
